...
ui <- dashboardPage(
  ...
  header = dashboardHeader(
    title = tagList(
      img(src = "logo.png", id = "logo-img", height = "35px", alt = "logo UFMT", style = "margin-left: 10px; margin-top: 7px; margin-bottom: 7px;")
    ),
    navbarMenu(
      id = "tabs",
      navbarTab(
    ...
    rightUi = tagList(
      tags$li(
        ...
        tags$a(
          class = "nav-link",
          href = "https://github.com/idmusis/dashboard-mestrado",
          ...
          lapply(list(
            ...
            filtro_box(
              ...
              column(
                width = 12, offset = 0,
                prettyCheckboxGroup(
                  inputId = paste0(args[[1]], "_selectall"),
                  label = NULL,
                  choiceValues = "selectall",
                  choiceNames = "Selecionar tudo",
                  selected = "selectall",
        ...
        column(
          ...
          lapply(seq_along(variaveis_filtradas), function(i) {
            ...
            if (length(mapping) > 0) {
              ...
              filtro_box(
                width = 12, headerBorder = TRUE, collapsible = TRUE, collapsed = if (i == 1) FALSE else TRUE,
    ...
    bs4DashBody(
      tags$head(
        tags$style(HTML("
          ...
          } /* Ajusta cabeçalho conforme tamanho da janela */
...
"))
      ...
      bs4TabItems(
        ...
        bs4TabItem(
          ...
          bs4Card(
            ...
            ### Configurações -----------------------
            fluidRow(actionButton(inputId = "controlbarToggle", label = "Filtros", icon = icon("filter"), class = "mx-2")), h3(""),
            prettyRadioButtons("criteria", "Critério de seleção do modelo",
              choices = c("BIC", "AIC"), selected = "BIC",
              animation = "smooth"
            ),
            prettyRadioButtons("direction", "Direção de seleção de variáveis",
              choiceNames = c("Eliminação bidirecional", "Seleção direta", "Eliminação reversa"), choiceValues = c("both", "forward", "backward"), selected = "both",
              animation = "smooth"
            ),
            prettyCheckbox("include_bootstrap",
              label = "Performar validação bootstrap", value = FALSE,
    ...
    <p><b>Período de aplicação do questionário:</b> 06 a 19/06/2024.</p>
        ...
        <h5>Links</h5>
    <p>O código-fonte deste dashboard está disponível em: <a href='https://github.com/idmusis/dashboard-mestrado' target='_blank' class='code-link'>https://github.com/idmusis/dashboard-mestrado</a>.</p>
...
server <- function(input, output, session) {
  ...
  plot_bar <- function(data, category_col, category_order = NULL, category_labels = NULL, xlab = "", ylab = "", tipo = "column", titulo = category_col,
    ...
    if (comparar == "comparar") {
      ...
      hc <- highchart() %>%
        ...
        hc_tooltip(
          shared = FALSE, useHTML = TRUE, headerFormat = '<span style="font-size: 0.8em">{point.key}</span><br/>',
      ...
      if (percent) {
        hc <- highchart() %>%
          ...
          hc_tooltip(
            shared = TRUE, useHTML = TRUE, headerFormat = '<span style="font-size: 0.8em">{point.key}</span><br/>',
  ...
  likert_chart <- function(data, category_col, group_col = "evadido",
    ...
    hc <- hchart(data_long, "bar", hcaes(x = !!sym(group_col), y = Contagem, group = Resposta)) %>%
      ...
      hc_title(text = titulo) %>%
      hc_xAxis(
        categories = unique(data_long[[group_col]]),
        title = list(text = "")
      ) %>%
      hc_yAxis(title = list(text = ""), labels = list(format = "{value}%")) %>%
  ...
  output$plot5 <-
    renderHighchart({
      ...
      plot_bar(
        dadosFiltrados(),
        category_col = "Em qual campus da UFMT você cursou seu último programa de pós-graduação stricto sensu?", titulo = "Câmpus"
      )
    })
  ...
  output$plot8 <- renderHighchart({
    force(forceRedraw())
    plot_bar(dadosFiltrados(),
      category_col = "Você fez uso de algum período de prorrogação de prazo para o término do seu curso de pós-graduação? Se sim, qual foi o período de prorrogação utilizado?",
  ...
  output$plot10 <- renderHighchart({
    force(forceRedraw())
    plot_bar(dadosFiltrados(),
      category_col = "Você trancou sua matrícula no programa de pós-graduação? Se sim, por quanto tempo?",
  ...
  output$plot15 <- renderHighchart({
    force(forceRedraw())
    plot_bar(dadosFiltrados(),
      category_col = "Estado civil ao iniciar o curso de pós-graduação",
  ...
  output$plot17 <- renderHighchart({
    force(forceRedraw())
    plot_bar(dadosFiltrados(),
      category_col = "Como você se identifica em relação à sua orientação sexual?", titulo = "Orientação sexual",
    ...
    if (req(input$comparar) == "total") {
      ...
      dados_barra <- dados_comparar %>%
        ...
        mutate(`Apresenta alguma deficiência ou transtorno? Marque todos que se aplicam` := factor(`Apresenta alguma deficiência ou transtorno? Marque todos que se aplicam`, levels = c(
          ...
          "Transtorno de Ansiedade Generalizada",
          "Outro",
          "Não, nenhuma deficiência ou transtorno"
        ))) %>%
        arrange(`Apresenta alguma deficiência ou transtorno? Marque todos que se aplicam`) %>%
        filter(`Apresenta alguma deficiência ou transtorno? Marque todos que se aplicam` != "Não, nenhuma deficiência ou transtorno") %>%
  ...
  output$plot20 <- renderHighchart({
    force(forceRedraw())
    plot_bar(dadosFiltrados(),
      category_col = "Qual era o rendimento per capita familiar durante o período em que realizou o curso de pós-graduação?", titulo = "Rendimento per capita familiar durante o curso",
      category_order = c(
        "Sem renda",
  ...
  output$plot22 <- renderHighchart({
    ...
    plot_pie(dadosFiltrados(), category_col = "Antes de seu desligamento do curso de pós-graduação, chegou a realizar exame de qualificação?") %>%
      hc_colors(c("#97979A", "#252528")) # Cores para evadidos
  })
  ...
  output$plot23 <- renderHighchart({
    ...
    plot_bar(dadosFiltrados(), category_col = "Após o desligamento nesse programa, você concluiu ou está cursando outro curso de pós-graduação stricto sensu?", category_order = c("Não concluí e não estou cursando", "Cursando na UFMT", "Cursando em outra instituição", "Concluí em outra instituição")) %>%
      hc_colors(c("#434348")) # Cores para evadidos
  })
  ...
  atualizarUI <- function() {
    ...
    output$info_criteria <- renderDT({
      info_criteria_df <- data.frame(
        ...
        LogLik = c(
          paste0(
            formatC(as.numeric(rv$info_criteria$LogLik_Stepwise), format = "f", digits = 2, decimal.mark = ","),
            " (g.l=", attr(rv$info_criteria$LogLik_Stepwise, "df"), ")"
          ),
          paste0(
            formatC(as.numeric(rv$info_criteria$LogLik_Nulo), format = "f", digits = 2, decimal.mark = ","),
    ...
    output$rocPlot <- renderHighchart({
      ...
      highchart() %>%
        ...
        hc_xAxis(
          title = list(text = "Especificidade"),
          min = -0.1, max = 1.1,
    ...
    output$configuracoes_modelo <- renderUI({
      HTML(paste0(
        ...
        "<b>Direção de seleção:</b> ", switch(rv$configuracoes_modelo$direcao,
          ...
          rv$configuracoes_modelo$direcao
        ), "<br>",
  ...
  observeEvent(input$analyze, {
    ...
    rv$configuracoes_modelo$variaveis_filtradas <- lapply(names(variaveis_filtradas), function(var) {
      id_checkbox <- variaveis_filtradas[[var]]$id
      selected_vals <- input[[id_checkbox]]

      selected_vals <- setdiff(selected_vals, "selectall")

      mapping <- Filter(function(m) m$var == var, dummy_mapping)[[1]]
